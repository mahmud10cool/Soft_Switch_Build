...
ylabel('Energy (J)')
xlabel('Time (ms)')
text(0.72*max(t_soft), ...
     min(regen_sim)+0.25*abs(min(regen_sim)), ...
...
ylabel('Energy (J)')
xlabel('Time (ms)')
text(0.72*max(t_energy), ...
     min(energy_fric)+0.25*abs(min(energy_fric)), ...
...
% Format all axes
ax = findall(comparison_fig,'Type','axes');
...
% Make ALL text bold:
% titles, x-labels, y-labels, annotations, etc.
txt = findall(comparison_fig,'Type','text');

set(txt, ...
    'FontName','Arial', ...
    'FontWeight','bold');

% Make legends bold if any are added later
lgd = findall(comparison_fig,'Type','legend');
...
comparison_fig = figure(2);
set(comparison_fig,'Position',[100 50 1000 1500]);
...
% Colors
sim_color  = [0.85 0.10 0.10];   % Red
prac_color = [0.00 0.45 0.74];   % Blue

line_width = 2;

% Prepare practical data

t_practical = t_pressure_exp(friction_index) - t_friction(1);
speed_fric = speed_exp_(friction_index);
...
energy_fric = cumsum(power_fric*1e-3);
t_energy = t_practical;

% Row 1 - Pressure

ax1 = nexttile;

plot(t_soft, pressure_sim, ...
    'Color',sim_color, ...
...
plot(t_soft, speed_sim, ...
    'Color',sim_color, ...
    'LineWidth',line_width);
hold on

plot(t_practical, speed_fric, ...
    'Color',prac_color, ...
    'LineWidth',line_width);
...
plot(t_soft, regen_torque_sim + friction_sim, ...
    'Color',sim_color, ...
    'LineWidth',line_width);
hold on
...
plot(t_soft, power_sim, ...
    'Color',sim_color, ...
    'LineWidth',line_width);
hold on

plot(t_practical, power_fric, ...
    'Color',prac_color, ...
    'LineWidth',line_width);
...
plot(t_soft, regen_sim, ...
    'Color',sim_color, ...
    'LineWidth',line_width);
hold on

plot(t_energy, energy_fric, ...
    'Color',prac_color, ...
    'LineWidth',line_width);
...
% Energy saved annotations

text(0.84*max(t_soft), ...
     -0.85, ...
     ['Simulation = ',num2str(regen_sim(end),'%.2f'),' J'], ...
     'Color',sim_color, ...
     'FontSize',10, ...
     'FontWeight','bold');
...
text(0.84*max(t_soft), ...
     -1.45, ...
     ['Practical = ',num2str(energy_fric(end),'%.2f'),' J'], ...
     'Color',prac_color, ...
     'FontSize',10, ...
     'FontWeight','bold');
...
set(comparison_fig,'Color','white');

% title(tl,'Simulation vs Practical Results', ...
%       'FontName','Arial', ...
%       'FontSize',20, ...
%       'FontWeight','bold', ...
%       'Color','black');

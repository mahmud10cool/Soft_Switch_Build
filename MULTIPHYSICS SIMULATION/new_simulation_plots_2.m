...
simulation = sim("soft_switch_new_simulation_2.slx");
t = simulation.tout;
...
innerEnergies = [Ework, Eloss];
innerNames    = {'Work','Loss'};

innerEnergies = max(innerEnergies, 0);
Ein = max(Ein, 0);

innerFrac = innerEnergies / max(Ein, eps);
...
legendLabels = [{sprintf('Input')},arrayfun(@(k) innerNames{k}, 1:numel(innerNames), 'UniformOutput', false)];

leg = legend(legendLabels, 'Location','best');
title(leg, "Energy:")
leg.FontSize = 14;

titl = title('Energy Breakdown');
titl.FontSize = 16;

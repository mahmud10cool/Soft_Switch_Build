...
%% Run Simulation
simulation = sim("Copy_8_of_SIMSCAPE_SIM_PROTOTYPE.slx");
...
label_out = [ "Throttling Losses",  "Pump Losses", "Electric Motor Losses", "Regenerated Energy"];
data_out = [Throttling_Losses(end) + Stored(end), Pump_Losses(end), elec_motor_losses(end), -Regen_Elec(end)];

label_inp = ["Hydraulic Energy In"];
data_inp = [ Hyd_Energy_In(end)];
